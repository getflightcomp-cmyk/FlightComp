...
// Priority order:
//   1. EU/EEA dep → EU261 (even if arr=TR or arr=CA)
//   2. UK dep → UK261
//   3. TR dep → SHY (even if arr=CA; TR dep to EU shows note about EU261 in results)
//   4. CA dep or arr → APPR
//   5. TR arr (non-EU/UK/CA dep) → SHY (assuming Turkish carrier)
//   6. fallback → EU261
export function detectRegulation(from, to) {
  const depInfo = resolveLocation(from);
  const arrInfo = resolveLocation(to);
  if (depInfo?.region === 'EU' || depInfo?.region === 'EEA') return 'EU261';
  if (depInfo?.region === 'UK') return 'UK261';
  if (depInfo?.region === 'TR') return 'SHY';
  if (depInfo?.region === 'CA' || arrInfo?.region === 'CA') return 'APPR';
  if (arrInfo?.region === 'TR') return 'SHY';
  return 'EU261';
}
...
export function assessClaim(answers) {
  ...
  const depCovered  = depInfo ? isCoveredRegion(depInfo.region) : null;
