...
function getCareRights(distanceKm, delayLength, isCancel, isCovered) {
  ...
  if (effectiveDelay >= careThreshold || isCancel) {
    rights.push({ emoji:'🍽️', titleKey:'care_meals_title', detailKey:'care_meals_eu_prop' });
    rights.push({ emoji:'📞', titleKey:'care_comm_title',  detailKey:'care_comm_eu' });
  }
  if (effectiveDelay >= 6 || isCancel) {
    rights.push({ emoji:'🏨', titleKey:'care_hotel_title',     detailKey:'care_hotel_eu' });
    rights.push({ emoji:'🚌', titleKey:'care_transport_title', detailKey:'care_transport_detail' });
  }
  if (effectiveDelay >= 5 || isCancel) {
    rights.push({ emoji:'↩️', titleKey:'care_rebook_title', detailKey:'care_rebook_eu' });
  }
  return rights;
}
...
export function assessClaimAPPR(answers) {
  ...
  if (validity.expired) {
    ...
    verdict = 'unlikely';
    if (isSafety) {
      verdictNote = 'verdict_note_appr_safety';
    } else {
  ...
  if (verdict !== 'unlikely' || reasonBlocks) {
    if (isDelayed || isCancel || isDenied) {
      careRights.push({ emoji:'🍽️', titleKey:'care_meals_title', detailKey:'care_meals_appr_3h' });
      careRights.push({ emoji:'📞', titleKey:'care_comm_title',  detailKey:'care_comm_appr' });
    }
    if ((isDelayed && ['6to9','9plus'].includes(apprDelayTier)) || isCancel || isDenied) {
      careRights.push({ emoji:'🏨', titleKey:'care_hotel_title',     detailKey:'care_hotel_appr' });
      careRights.push({ emoji:'🚌', titleKey:'care_transport_title', detailKey:'care_transport_detail' });
    }
    careRights.push({ emoji:'↩️', titleKey:'care_rebook_title', detailKey:'care_rebook_appr' });
  }
...
function getCompensationSHY(isDomestic, distanceKm) {
  if (isDomestic) return { amount: '€100', raw: 100, currency: 'EUR', payableInTRY: true, isDomestic: true };
  if (distanceKm === null) return { amount: '€250–€600', raw: null, currency: 'EUR', payableInTRY: true };
  if (distanceKm <= 1500) return { amount: '€250', raw: 250, currency: 'EUR', payableInTRY: true };
  if (distanceKm <= 3500) return { amount: '€400', raw: 400, currency: 'EUR', payableInTRY: true };
  return { amount: '€600', raw: 600, currency: 'EUR', payableInTRY: true };
}
...
export function assessClaimSHY(answers) {
  ...
  if (validity.expired) {
    ...
    verdict = 'unlikely';
    verdictNote = alsoCoveredByEU261 ? 'verdict_note_shy_delay_eu261' : 'verdict_note_shy_delay';
  } else if (isForceMajeure) {
    verdict = 'unlikely';
    verdictNote = 'verdict_note_shy_forcemajeure';
  ...
  if (delayHours >= 2 || isCancel || isDenied) {
    careRights.push({ emoji:'🍽️', titleKey:'care_meals_title', detailKey:'care_meals_shy_2h' });
    careRights.push({ emoji:'📞', titleKey:'care_comm_title',  detailKey:'care_comm_shy' });
  }
  if (delayHours >= 5 || isCancel || isDenied) {
    careRights.push({ emoji:'🏨', titleKey:'care_hotel_title',     detailKey:'care_hotel_shy' });
    careRights.push({ emoji:'🚌', titleKey:'care_transport_title', detailKey:'care_transport_detail' });
    careRights.push({ emoji:'↩️', titleKey:'care_rebook_title',    detailKey:'care_rebook_shy' });
  }
  ...
  return {
    ...
    deskScript,
    shyMeta: {
      deadlineKey:   'shy_deadline',
      escalationKey: 'shy_escalation_dgca',
    },
    claimData: {
      flightNumber,
      flightDate,
...
export function assessClaim(answers) {
  ...
  if (validity.expired) {
    ...
    verdict = 'unlikely';
    verdictNote = 'verdict_note_eu_extraordinary';
