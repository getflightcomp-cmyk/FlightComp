...
const ESCALATION_AUTHORITIES = {
  ...
  SHY: {
    name: 'Directorate General of Civil Aviation (SHGM — Sivil Havacılık Genel Müdürlüğü)',
...
const EU_NEB = {
  FR: { name: 'Direction Générale de l\'Aviation Civile (DGAC)', url: 'https://www.ecologie.gouv.fr/dgac' },
  DE: { name: 'Luftfahrt-Bundesamt (LBA)', url: 'https://www.lba.de' },
  NL: { name: 'Inspectie Leefomgeving en Transport (ILT)', url: 'https://www.ilent.nl' },
  ES: { name: 'Agencia Estatal de Seguridad Aérea (AESA)', url: 'https://www.seguridadaerea.gob.es' },
  IT: { name: 'Ente Nazionale per l\'Aviazione Civile (ENAC)', url: 'https://www.enac.gov.it' },
  BE: { name: 'Directorate-General Air Transport (DGTA)', url: 'https://mobilit.belgium.be' },
  PT: { name: 'Autoridade Nacional da Aviação Civil (ANAC)', url: 'https://www.anac.pt' },
  IE: { name: 'Commission for Aviation Regulation (CAR)', url: 'https://www.aviationreg.ie' },
  DK: { name: 'Danish Transport Authority', url: 'https://www.trafikstyrelsen.dk' },
  SE: { name: 'Swedish Transport Agency', url: 'https://www.transportstyrelsen.se' },
  FI: { name: 'Finnish Transport and Communications Agency (Traficom)', url: 'https://www.traficom.fi' },
  NO: { name: 'Civil Aviation Authority Norway', url: 'https://luftfartstilsynet.no' },
  PL: { name: 'Urząd Lotnictwa Cywilnego (ULC)', url: 'https://www.ulc.gov.pl' },
  AT: { name: 'Agentur für Passagier- und Fahrgastrechte (apf)', url: 'https://www.apf.gv.at' },
  GR: { name: 'Hellenic Civil Aviation Authority (HCAA)', url: 'https://www.hcaa.gr' },
  CZ: { name: 'Civil Aviation Authority of Czech Republic', url: 'https://www.caa.cz' },
  HU: { name: 'Hungarian Aviation Authority', url: 'https://www.nkh.gov.hu' },
  RO: { name: 'Romanian Civil Aeronautical Authority (RCAA)', url: 'https://www.caa.ro' },
  HR: { name: 'Croatian Civil Aviation Agency', url: 'https://www.ccaa.hr' },
};
...
/**
 * Look up airline contact info by IATA code.
 * Falls back to extracting the code from a flight number string.
 */
export function getAirlineContact(iataCodeOrFlightNum) {
  if (!iataCodeOrFlightNum) return null;
  const raw = iataCodeOrFlightNum.trim().toUpperCase();
  // Try direct code first (2-3 chars)
  if (raw.length <= 3 && AIRLINE_CONTACTS[raw]) return AIRLINE_CONTACTS[raw];
  // Try first 2 chars of flight number
  const prefix = raw.replace(/\s+/g, '').slice(0, 2);
  return AIRLINE_CONTACTS[prefix] || null;
}
...
export function getEscalationAuthority(regulation, depInfo) {
  ...
  if (regulation === 'EU261' && depInfo?.country) {
    ...
    if (neb) return { ...neb, note: 'File a free complaint with your departure country\'s NEB if the airline rejects or ignores your claim.' };
  }
  return {
    name: 'relevant National Enforcement Body (NEB)',
    url: 'https://ec.europa.eu/transport/modes/air/passengers_rights',
    note: 'Contact the NEB of the departure country to file a free complaint.',
  };
}
